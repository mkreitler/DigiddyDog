tj.MetaBlocks.Path = function(colorIn) {
  var cells = [],
      color = colorIn,
      aabb = new tj.MathEx.rect2(0, 0, 0, 0),
      nCells = 0,
      active = false,
  ...
  this.onCreate = function() {
    ...
    for (i=0; i<tj.MB.constants.MAX_CELLS_PER_PATH; ++i) {
      cells.push({row:-1, col:-1, dirX: tj.MB.constants.DIR.UNKNOWN, dirY: tj.MB.constants.DIR.UNKNOWN, speed: 1});
    }
  };
  this.onCreate();
  ...
  this.xDirAtCell = function(iCell) {
    var newDir = tj.MB.constants.DIR.UNKNOWN;
  ...
  this.addCellRowCol = function(row, col, dirX, dirY) {
    ...
    if (!bAdded) {
      if (nCells < tj.MB.constants.MAX_CELLS_PER_PATH || bTruncated) {
        if (nCells >= 0) {
  ...
  this.updateBounds = function() {
    var i = 0,
        minX = tj.MB.constants.COLS,
        maxX = 0,
        minY = tj.MB.constants.ROWS,
        maxY = 0;
    ...
    aabb.x = minX;
    aabb.y = minY;
    aabb.w = Math.max(0, maxX - minX + 1);
    aabb.h = Math.max(0, maxY - minY + 1);
  };

  this.draw = function(gfx, cellDx, cellDy) {
    var i = 0,
        x = 0,
        y = 0,
        halfSize = 0,
    ...
    if (active && nCells > 0) {
      ...
      halfSize = Math.floor(cellDx - tj.MB.constants.BORDER_WIDTH) * 0.5;
      fifthSize = Math.round(halfSize * 2.0 / 5.0);

      gfx.fillStyle = color;
      for (i=0; i<nCells; ++i) {
        x = Math.floor(cells[i].col * cellDx + cellDx * 0.5 + tj.MB.constants.BORDER_WIDTH * 0.5);
        y = Math.floor(cells[i].row * cellDy + cellDy * 0.5 + tj.MB.constants.BORDER_WIDTH * 0.5);
        ...
        gfx.lineTo(-fifthSize, 2 * fifthSize);
        gfx.closePath();
  //      gfx.arc(0, 0, (cellDx + cellDy) * 0.5 / 2 * RADIUS_FACTOR, 0, 2.0 * Math.PI, true);
        gfx.fill();
